...
<!-- USB_PACKET_TABLE_LINK_20260913 -->

## 从接入到拔出的三列逐包表

已将同一全速键盘协议实例整理为[完整三列逐包表](USB全速键盘_接入到拔出_三列逐包表.md)：第一列为包类型，第二列逐域列出SYNC至EOP的全部内容、位填充及NRZI线路序列，第三列解释用途。含全部202个包（40个SOF），并按时间列出连接、复位、每段包间空闲和拔出。示例设定已在表前说明。
